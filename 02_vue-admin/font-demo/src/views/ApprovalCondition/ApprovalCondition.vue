<template>
  <div class="vacation-approval">
    ...
    <div class="base-table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        <el-table-column v-for="item in columns" :key="item.label" :label="item.label" :prop="item.prop"
          :formatter="item.formatter" :width="item.width||'160px'">
        </el-table-column>
        <el-table-column fixed="right" label="Operation" width="120px">
          <template #default="scope">
            <el-button size="small" type="primary"
              v-if="userName === scope.row.curAuditUserName && [1,2].includes(scope.row.applyState)"
              v-permission="'approve-check'" @click="handleCheck(scope.row)">Check
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <CheckDialog @refreshList="refreshList" ref="checkLeaveDialog" :detail="currentData">
    </CheckDialog>
  </div>
</template>
...
const userName = sotre.state.userInfo.userName;
// search all data
...
const handleCheck = (row) => {
  ...
  data.remark = null;
  data.applyName = row.applyUser.userName;
  data.applyTypeName = {
    1: "VacationThing",
    2: "TiaoXiu",
    3: "WinterVacation",
  ...
  proxy.$nextTick(() => {
    proxy.$refs.checkLeaveDialog.dialogVisible = true;
  });
}
const handleApplys = () => {
